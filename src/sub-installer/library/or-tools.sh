...
sudo cmake "${CMAKE_ENVIRONMENT[@]}" \
    -DBUILD_CXX:BOOL=ON \
    -DBUILD_FLATZINC:BOOL=ON -DBUILD_BZip2:BOOL=ON -DBUILD_ZLIB:BOOL=ON -DBUILD_Protobuf:BOOL=ON -DBUILD_re2:BOOL=ON \
    ...
    -DUSE_SCIP:BOOL=ON -DBUILD_SCIP:BOOL=ON -DBUILD_soplex:BOOL=ON \
